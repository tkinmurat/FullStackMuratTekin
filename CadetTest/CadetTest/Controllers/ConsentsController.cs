...
namespace CadetTest.Controllers
{
    ...
    public class ConsentsController : ControllerBase
    {
        ...
        [AllowAnonymous]
        [HttpPost("AddNewConsent")]
        public IActionResult AddNewConsent(NewConsent newConsent)
        {
            var cevap = _dataService.AddConsents(newConsent);
            return Ok(cevap);
        }


        [AllowAnonymous]
        [HttpPost("DeleteConsent")]
        public IActionResult DeleteConsent(Consent consentToDelete)
        {
            var cevap = _dataService.DeleteConsent(consentToDelete);
            return Ok(cevap);
        }

        [AllowAnonymous]
        [HttpPost("UpdateConsent")]
        public IActionResult UpdateConsent(Consent updateConsent)
        {
            var cevap = _dataService.UpdateConsent(updateConsent);
            return Ok(cevap);
        }
